...
export class VehicleForm {
  ...
  constructor() {
    effect(() => {
      if (this.isEditing() && this.vehicle()) {
        ...
        // Convert number price to string for form
  ...
  protected readonly form = this.formBuilder.nonNullable.group({
    ...
    year: ['', [Validators.required, Validators.min(1980), Validators.max(2025)]],
    color: ['', Validators.required],
    price: ['', [Validators.required, Validators.min(50000000), Validators.max(250000000)]],
